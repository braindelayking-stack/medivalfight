...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('composure-defense')
        .setDescription('🧘 Reduce damage on low health!'),
    async execute(interaction) {
        ...
        const ability = db.prepare('SELECT * FROM user_abilities WHERE user_id = ? AND server_id = ? AND ability_id = ?').get(userId, serverId, 'composure_defense');
        if (!ability) {
        ...
        if (bossCommand.activeBossFights.has(bossFightId)) {
            ...
            // Activate composure defense
            state.userEffects.composure = { active: true, damageReduction: 0.1 };
            state.log.push('🧘 You activated composure defense! When below 20% HP, take 10% less damage!');
            ...
            await interaction.reply({ content: '🧘 You used composure defense!', ephemeral: true });
            return;
        }
