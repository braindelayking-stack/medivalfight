...
// Cooldown tracker (5 seconds)
const cooldowns = new Map();

module.exports = {
    data: new SlashCommandBuilder()
        .setName('smash')
        .setDescription('💥 Deal high damage and shatter boss shields, but take extra damage next hit'),
    async execute(interaction) {
        ...
        const ability = db.prepare('SELECT * FROM user_abilities WHERE user_id = ? AND server_id = ? AND ability_id = ?').get(userId, serverId, 'smash');
        if (!ability) {
        ...
        if (bossCommand.activeBossFights.has(bossFightId)) {
            ...
            // Shatter boss shield
            state.bossEffects.shield = { active: false, blockChance: 0 };
            ...
            state.log.push(`💥 You used Smash, dealing ${damage} damage and shattering shields! You'll take 1.2x damage on the next hit!`);

            // Apply increased damage taken effect
            state.userEffects.increasedDamageTaken = { active: true, multiplier: 1.2 };
            ...
            if (state.bossHP <= 0) {
                ...
                await interaction.reply({ content: '💥 You used Smash!', ephemeral: true });
                return;
            }
