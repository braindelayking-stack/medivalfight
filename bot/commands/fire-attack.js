...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('fire-attack')
        .setDescription('🔥 Use fire attack in battle!'),
    async execute(interaction) {
        ...
        const ability = db.prepare('SELECT * FROM user_abilities WHERE user_id = ? AND server_id = ? AND ability_id = ?').get(userId, serverId, 'fire_attack');
        if (!ability) {
        ...
        if (bossCommand.activeBossFights.has(bossFightId)) {
            ...
            if (!blocked) {
                ...
                state.log.push(`🔥 You used fire attack, dealing ${damage} damage!`);
            }
            ...
            await interaction.reply({ content: '🔥 You used fire attack!', ephemeral: true });
            return;
        }
