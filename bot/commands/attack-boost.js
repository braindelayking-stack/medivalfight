...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('attack-boost')
        .setDescription('⚡ Boost your next attack!'),
    async execute(interaction) {
        ...
        const ability = db.prepare('SELECT * FROM user_abilities WHERE user_id = ? AND server_id = ? AND ability_id = ?').get(userId, serverId, 'attack_boost');
        if (!ability) {
        ...
        if (bossCommand.activeBossFights.has(bossFightId)) {
            ...
            state.userEffects.attackBoost = { active: true, multiplier: 1.1 };
            state.log.push('⚡ You activated attack boost! Next attack deals 10% more damage!');
            ...
            await interaction.reply({ content: '⚡ You used attack boost!', ephemeral: true });
            return;
        }
