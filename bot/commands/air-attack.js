...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('air-attack')
        .setDescription('💨 Use air attack in battle!'),
    async execute(interaction) {
        ...
        const ability = db.prepare('SELECT * FROM user_abilities WHERE user_id = ? AND server_id = ? AND ability_id = ?').get(userId, serverId, 'air_attack');
        if (!ability) {
        ...
        if (bossCommand.activeBossFights.has(bossFightId)) {
            ...
            if (!blocked) {
                ...
                state.log.push(`💨 You used air attack, dealing ${damage} damage!`);
            }
            ...
            cooldowns.set(userId, now + 3000);
            await interaction.reply({ content: '💨 You used air attack!', ephemeral: true });
            return;
        }
